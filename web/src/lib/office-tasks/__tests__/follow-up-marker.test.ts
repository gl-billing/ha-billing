...
describe("applyFollowUpWithNote", () => {
  ...
  it("reads earlier HA sheet follow-up markers", () => {
    expect(getFollowUpFromRemarks("note\nGL_FOLLOW_UP:Waiting")).toBe("Waiting");
    expect(getFollowUpFromRemarks("HA_FOLLOW_UP:Started")).toBe("Started");
  });
});
