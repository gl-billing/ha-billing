...
export function displayEventRemarks(remarks: string): string {
  return String(remarks || "")
    ...
    .replace(/\n?FILING_DRAFT_TASK:[A-Z0-9-]+/gi, "")
    .replace(/\n?FILING_REVIEW_TASK:[A-Z0-9-]+/gi, "")
    .replace(/\n?FILING_EXHIBITS_TASK:[A-Z0-9-]+/gi, "")
    .replace(/\n?FILING_FILING_TASK:[A-Z0-9-]+/gi, "")
    .replace(/\n?FILING_SERVE_TASK:[A-Z0-9-]+/gi, "")
    .replace(/\n?FILING_PROOF_TASK:[A-Z0-9-]+/gi, "")
    .replace(/\n?FILING_STAGE_(DRAFTING|REVIEW|EXHIBITS|FILING|SERVING|PROOF)/gi, "")
    .replace(/\n?FILING_WORKFLOW_STAGE:[a-z0-9_-]+/gi, "")
    .replace(/\n?FILING_DRAFT_DOC:\S+/gi, "")
    .replace(/\n?FILING_DRAFT_NOTE:[^\n]+/gi, "")
    .replace(/\n?FILING_ROUTE_OVERRIDE:[^\n]+/gi, "")
