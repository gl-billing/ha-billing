...
describe("staff cash advances", () => {
  ...
  it("creates payroll deductions for the current month only", () => {
    ...
    const june = buildCashAdvanceAdjustmentsForMonth([advance], "liaison", 2026, 6);
    expect(june).toHaveLength(2);
    expect(june[0].amount).toBe(-1000);
    expect(june[0].label).toBe("Cash advance · 1/4");

    const july = buildCashAdvanceAdjustmentsForMonth([advance], "liaison", 2026, 7);
  ...
  it("marks installments paid when a pay run is recorded", () => {
    ...
    const updated = markCashAdvanceInstallmentsPaid([advance], "liaison", 2026, 6, "mid", "2026-06-15");
    expect(updated[0].installments[0].paidAt).toBe("2026-06-15");
    expect(cashAdvanceRemainingBalance(updated[0])).toBe(1500);
    expect(updated[0].status).toBe("active");

    const paid = markCashAdvanceInstallmentsPaid(updated, "liaison", 2026, 6, "end", "2026-06-30");
    const paid2 = markCashAdvanceInstallmentsPaid(paid, "liaison", 2026, 7, "mid", "2026-07-15");
    const paid3 = markCashAdvanceInstallmentsPaid(paid2, "liaison", 2026, 7, "end", "2026-07-31");
