...
import { markStaff13thMonthPaid } from "@/lib/sheets/staff-salary";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      report,
      message: `${year} 13th month pay marked for ${report.staffName}.`
    });
  } catch (error) {
    if (isQuotaError(error)) {
      return NextResponse.json({ error: quotaErrorMessage() }, { status: 429 });
    }
    const message = error instanceof Error ? error.message : "Failed to mark 13th month pay.";
    const status =
      message.startsWith("Unauthorized") || message.includes("firm admins")
        ? 403
        : message.includes("already marked") || message.includes("Unknown") || message.includes("greater than zero")
          ? 409
          : 500;
